...
| `jointStyle.ts` | How a rigid (anchor) union draws: socket TEE (lone ~90° on-body branch — exactly 3 pipe ends), wrap-arrow pin (other angles), or ONE brown fabricated-union SPHERE per node when >3 pipe ends enter the point (solved X / many-way crossings) | `anchorRendersAsTee`, `anchorRendersAsHub`, `junctionEndCount` |
