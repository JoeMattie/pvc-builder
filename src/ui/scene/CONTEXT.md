...
| `SceneLabels.tsx` | Viewport semantic labels | BOM `P#` cut IDs when `editorStore.sceneStatus === 'fabricate'`; selected/hover labels for pipes, joints, fittings/conflicts; positions are updated imperatively from eased node positions; fabricate labels project through the camera and HIDE when inside UI rects (`[data-floating-island]`, `[data-viewport-occluder]`, menus/popovers); drei `Html zIndexRange={[18, 0]}` keeps them under chrome |
