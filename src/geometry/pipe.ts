// Shared pipe geometry. Pure functions used by BOTH rendering (formed-pipe
// sweeps) and the BOM (developed lengths, bend schedule), so the
// developed-length definition lives in exactly one place. Fully 3D: all inputs
// are Vec3 polylines; every quantity here is intrinsic to the polyline
// (lengths, turn angles, relative bend planes) and therefore invariant under
// rigid transforms. Copied verbatim from riglab (planfile §2).
 ...
 * the (i+1)-th point, matching the schema's length = points.length − 2). A
...
// ── Bend-plane rotation ("twist") convention ────────────────────────────────
// Each bend of a heat-bent pipe lies in a plane (spanned by its incoming and
// outgoing segments). To fabricate a 3D formed pipe, the builder makes bend 1,
// then ROTATES THE PIPE IN THE BENDER about the straight segment between bends
// before making bend 2, and so on. bendDihedralsRad reports that rotation per
// interior vertex:
//   • First bend: 0 by convention — the builder orients the first bend plane
//     freely; everything after is measured relative to it.
//   • Bend i: the signed dihedral angle from the PREVIOUS bend's plane to this
//     bend's plane, measured about the shared straight segment, with the axis
//     direction pointing from the previous vertex toward this vertex (the
//     direction of travel along the pipe). Sign = right-hand rule about that
//     axis.
//   • Degenerate vertices (collinear or reversal segments) have no bend plane:
//     they report 0 and do NOT update the reference plane.
// A planar polyline reports all zeros (0 for same-way bends, ±π for an S-bend,
// which is exactly "flip the pipe over in the bender").
// ─────────────────────────────────────────────────────────────────────────────

/** Per-interior-vertex bend-plane rotation of a 3D polyline (radians), array
 * aligned with the schema's filletRadiiM (length = points.length − 2). See the
 * convention note above: first bend 0, sign = right-hand rule about the shared
 * segment's travel direction, degenerate vertices 0. */
